...
module.exports = () => {
  this.getAll = loteria =>
    modelHelper.getList(db.collection(`numero_${loteria}`).orderBy("id"));

  this.getSingle = (loteria, numero) =>
    modelHelper.getSingle(db.collection(`numero_${loteria}`).doc(numero));

  this.update = (loteria, numero, record) => {
    return new Promise(resolve => {
      db.collection(`numero_${loteria}`)
        .doc(numero)
        .set(record)
        .then(() => {
